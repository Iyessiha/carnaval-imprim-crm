...
type Client = { id: string; nom: string; template_fne_defaut: string }
...
export default function DevisClient({ devis: initial, clients, produits, tauxTva }: {
  ...
  const save = async () => {
    ...
    if (sel?.id) {
      ...
      if (devis_lignes?.length) {
        await sb.from('devis_lignes').insert(devis_lignes.map((l: Ligne, i: number) => ({ ...l, devis_id: sel.id, ordre: i })))
      }
      ...
      if (e) { setError(e.message); setLoading(false); return }
      if (devis_lignes?.length) {
        await sb.from('devis_lignes').insert(devis_lignes.map((l: Ligne, i: number) => ({ ...l, devis_id: created.id, ordre: i })))
      }
      const cNom = clients.find(c => c.id === form.client_id)?.nom || ''
  ...
  const toFacture = async (d: Devis) => {
    ...
    if (d.devis_lignes?.length) {
      await sb.from('factures_lignes').insert(d.devis_lignes.map((l: Ligne, i: number) => ({ ...l, facture_id: facture.id, taxes: ['TVA'], ordre: i })))
    }
  ...
  return (
    <div style={{ padding: 24 }}>
      <PageHeader
        title="Devis" subtitle={`${devis.length} devis`}
          ...
          {filtered.map(d => {
            ...
            return (
              <tr key={d.id}>
                ...
                <td style={{ ...td, whiteSpace: 'nowrap', color: expire ? '#D14343' : '#7A736C' }}>{d.validite ? formatDateFR(d.validite) : '—'}{expire ? ' ⚠️' : ''}</td>
                <td style={{ ...td, fontWeight: 700 }}>{formatFCFA(t.ttc)}</td>
                <td style={td}><Badge value={d.statut} /></td>
                <td style={{ ...td, textAlign: 'right', whiteSpace: 'nowrap' }}>
                  <BtnIcon onClick={() => openView(d)}><Eye size={16} /></BtnIcon>
                  <BtnIcon onClick={() => openEdit(d)}><Pencil size={16} /></BtnIcon>
                  {d.statut !== 'Accepté' && d.statut !== 'Refusé' && <BtnIcon onClick={() => toFacture(d)}><Receipt size={16} /></BtnIcon>}
      ...
      {modal === 'view' && sel && (
        <Modal title={`Devis ${sel.numero}`} onClose={() => { setModal(null); setSel(null) }} wide>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
            <div style={{ fontWeight: 700, fontSize: 15 }}>{sel.clients?.nom}</div>
            <div style={{ textAlign: 'right', fontSize: 13 }}>
              <div>{formatDateFR(sel.date)}</div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 14 }}>
            ...
            {sel.statut !== 'Accepté' && <BtnPrimary onClick={() => toFacture(sel)} disabled={loading}><Receipt size={16} /> → Facture</BtnPrimary>}
          </div>
        </Modal>
      )}
    </div>
