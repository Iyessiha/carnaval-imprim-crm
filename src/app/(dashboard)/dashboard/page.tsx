...
export default async function DashboardPage() {
  ...
  const facStats = (factures || []).map((f) => {
    const t = calculerTotaux(f.factures_lignes || [], f.remise || 0, tva, f.tva_applicable)
    const paye = (f.paiements || []).reduce((s: number, p: { montant: number }) => s + p.montant, 0)
  ...
  const prodEnCours  = (productions || []).filter((p) => ['En attente', 'En production'].includes(p.statut)).length
  ...
  const kpis = [
    ...
    { label: "Devis en attente",     value: devisEnCours,          color: '#2A5FA5', bg: '#e5edf8', icon: '📄' },
    { label: "Productions actives",  value: prodEnCours,           color: '#F39200', bg: '#fef3e2', icon: '🖨️' },
  ]

  const prodsActives = (productions || []).filter((p) => p.statut !== 'Livré').slice(0, 5)
          ...
          prodsActives.map((p) => (
            <div key={p.id} style={{ padding: 12, border: '1px solid #E4DDD6', borderRadius: 12, marginBottom: 10 }}>
              <div style={{ fontWeight: 600, fontSize: 13, marginBottom: 6, lineHeight: 1.35 }}>
                {p.caracteristique?.slice(0, 70)}{(p.caracteristique?.length || 0) > 70 ? '…' : ''}
              </div>
