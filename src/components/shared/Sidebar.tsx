...
import {
  ...
  Package, Truck, Settings, LogOut, BarChart3, AlertCircle, ShoppingCart
} from 'lucide-react'
    ...
    <aside style={{
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 1, flex: 1 }}>
        {nav.map((item, idx) => {
          if ('divider' in item) {
            return (
              <div key={idx} style={{ fontSize: 9, fontWeight: 800, color: '#C2117A', textTransform: 'uppercase', letterSpacing: '1.5px', padding: '10px 8px 4px', marginTop: 4 }}>
                {item.divider}
              </div>
            )
          }
